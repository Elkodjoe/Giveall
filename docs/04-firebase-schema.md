...
- `validateProfile` — Firestore trigger on `profiles/{userId}` **creation** (not every write — `attachmentScores` is set once at onboarding and never touched again; the nightly job only updates `loveLanguageWeights`). Writes a `validation_events` doc if `attachmentScores._sum` drifts more than ±0.02 from 1.0, rather than only logging to the Cloud Functions console.
