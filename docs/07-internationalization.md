...
- **iOS App Store language metadata** (`app.json`'s `ios.infoPlist.CFBundleLocalizations`) declares the 7 supported languages, so the App Store listing can show "available in 7 languages." **Android has no equivalent declared** — Google Play infers supported languages from the APK's actual resource localization (`res/values-*`), which this app doesn't use since translation happens entirely at the JS/i18next layer, not via native Android string resources. The app's language switching itself works identically on both platforms regardless; this only affects each store listing's own "Languages" metadata.
